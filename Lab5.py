...
def producer() -> None:
    ...
    for _ in range(SIZE * 2): #we just produce twice the buffer size for testing
        ...
        if (empty != 0):     #wait until the buffer is not completely full to activate producer
            empty.acquire()         #acquire semaphore empty
            mutex.acquire()         #acquire lock mutex to avoid race condition problem
...
def consumer() -> None:
    ...
    for _ in range(SIZE * 2): #we just consume twice the buffer size for testing
        ...
        if (full != SIZE):               #wait until the buffer is not completely full
            full.acquire()               #acquire semaphore full
            mutex.acquire()              #acquire lock mutex to avoid race condition problem 
...
if __name__ == "__main__":
    ...
    producer_thread = threading.Thread(target = producer)             #initializing threads
