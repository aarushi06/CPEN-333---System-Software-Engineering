...
def philosopher(id: int, chopstick: list): 
    ...
    for _ in range(6): #to make testing easier, instead of a forever loop we use a finite loop
        ...
        if (chopstick[leftChopstick].get_value() == 1 and chopstick[rightChopstick].get_value() == 1):  #if both chopsticks are not acquired
        ...
        elif (chopstick[leftChopstick].get_value() == 0 and chopstick[rightChopstick].get_value() == 1): 
            print(f"DEBUG: philosopher{id} does not have access to chopstick{leftChopstick}, waiting")

        elif (chopstick[leftChopstick].get_value() == 1 and chopstick[rightChopstick].get_value() ==0 ):
            print(f"DEBUG: philosopher{id} does not have access to chopstick{rightChopstick}, waiting")
