...
def philosopher(id: int, chopstick: list): 
    ...
    for _ in range(6): #to make testing easier, instead of a forever loop we use a finite loop
        ...
        if (id % 2 != 0):
            chopstick[rightChopstick].acquire()  #right chopstick for us is the philosopher's left chopstick
            ...
            chopstick[leftChopstick].acquire()   #left chopstick for us is the philosopher's right chopstick
            print(f"DEBUG: philosopher{id} has chopstick{leftChopstick}")
